...
export const getOrdersWithStages = async (): Promise<Order[]> => {
  try {
    const response = await fetch(`http://92.242.60.192:3000/api/orders`);
  ...
  try {
    ...
    if (stageData.files && stageData.files.length > 0) {
      for (const file of stageData.files.slice(0, 3)) { // Максимум 3 изображения
        ...
        const uploadResponse = await fetch(`http://92.242.60.192:3000/api/uploadPhotoStage`, {
          method: 'POST',
          body: formData
        });
    ...
    // 3. Создаем этап
    const response = await fetch(`http://92.242.60.192:3000/api/stages`, {
      method: 'POST',
      headers: {
